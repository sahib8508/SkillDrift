...
NOTE: Uses st.camera_input (built-in Streamlit) instead of
streamlit-webrtc to avoid the tornado dependency crash on
...
def _process_frame(img_bytes: bytes):
    """Process a JPEG frame from st.camera_input and update state."""
    now = time.time()
...
# =============================================================
# CAMERA WIDGET  (replaces webrtc_streamer — no tornado needed)
# =============================================================

def render_proctor_camera(key: str = "skilldrift-proctor"):
    """Render the proctoring camera widget using st.camera_input.

    Drop-in replacement for the old webrtc_streamer call.
    No tornado, no streamlit-webrtc, no extra dependencies.
    Returns True (truthy) when active so callers that do `if ctx:`
    continue to work unchanged.
    """
    try:
        snapshot = st.camera_input(
            label="Proctoring Camera",
            key=key,
            label_visibility="collapsed",
        )

        if snapshot is not None:
            _process_frame(snapshot.getvalue())
            with _LOCK:
                _STATE["running"] = True
...
def get_proctor_camera_active() -> bool:
    """Return True if the camera has processed at least one frame."""
    with _LOCK:
